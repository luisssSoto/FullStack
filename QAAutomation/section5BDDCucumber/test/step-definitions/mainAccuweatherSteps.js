...
When(/^The user inputs '(.*)' in the search field$/, async(cityName) => {
    AllureReporter.addStep("Click on the input field");
    await AccuWeatherMainPage.clickOnInputField();
    AllureReporter.addStep("Set the input field");
    await AccuWeatherMainPage.setInputField(cityName);
});
Then('Search results list is displayed', async() => {
    AllureReporter.addStep("Check if the list was displayed");
    assert.isTrue(await AccuWeatherMainPage.isSearchResultsDisplayed());
});
...
Then('The web page is opened', async() => {
    ...
    assert.isTrue(await AccuWeatherMainPage.isPageOpened());
});
